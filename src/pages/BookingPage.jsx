...
const APT_TYPES = [
  { value: 'studio', label: 'Студия',     icon: '🏠', desc: '~2 часа' },
  { value: '1br',    label: '1 спальня',  icon: '🛏',  desc: '~4 часа' },
  { value: '2br',    label: '2 спальни',  icon: '🛏🛏', desc: '~6 часов' },
  { value: '3br',    label: '3 спальни',  icon: '🏡',  desc: '~8 часов' },
  { value: 'house',  label: 'Дом',        icon: '🏘',  desc: '~10 часов' },
]

const STEPS = ['Адреса', 'Детали', 'Контакты', 'Готово']
...
export default function BookingPage() {
  ...
  const next = () => {
    ...
    if (step === 2 && (!form.full_name || !form.phone)) {
      setError('Пожалуйста, укажите имя и телефон'); return
    }
    setStep(s => s + 1)
  }

  const submit = async () => {
    if (!form.full_name || !form.phone) { setError('Укажите имя и телефон'); return }
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '24px 16px 40px' }}>
        ...
        {step === 0 && (
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', margin: '0 0 8px', letterSpacing: '-0.5px' }}>
              Закажите переезд онлайн
            </h1>
            <p style={{ fontSize: 15, color: '#6B7280', margin: 0 }}>
              Заполните форму — мы свяжемся с вами в течение 30 минут
            </p>
          </div>
        )}
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: 24, boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
          ...
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={labelStyle}>📅 Дата переезда</label>
                <input style={inputStyle} type="date"
                  min={new Date().toISOString().split('T')[0]}
                  value={form.move_date} onChange={e => set('move_date', e.target.value)} />
              </div>
              <div>
                <label style={labelStyle}>📏 Расстояние (примерно, миль)</label>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div>
                <label style={labelStyle}>👷 Количество грузчиков</label>
              ...
              <div style={{ background: 'linear-gradient(135deg, #EFF6FF, #F5F3FF)', borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#6B7280', marginBottom: 4 }}>
                  <span>Труд ({form.movers_count} чел. × {price.hours}ч × ${price.rate}/ч)</span>
                  <span>${price.rate * price.hours}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#6B7280', marginBottom: 4 }}>
                  <span>Транспорт</span><span>${price.travelFee}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#6B7280', marginBottom: 10 }}>
                  <span>Материалы</span><span>${price.materialsFee}</span>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <textarea style={{ ...inputStyle, resize: 'none', height: 80 }}
                  placeholder="Есть ли лифт? Хрупкие вещи? Пианино?"
              ...
              <div style={{ background: '#F9FAFB', borderRadius: 14, padding: 14, fontSize: 13, color: '#6B7280' }}>
                ...
                <div style={{ marginBottom: 4 }}>📅 {form.move_date} · {APT_TYPES.find(t=>t.value===form.apt_type)?.label} · {form.movers_count} грузчика</div>
                <div style={{ fontWeight: 700, color: '#3B82F6', fontSize: 16, marginTop: 8 }}>Итого: ${price.total}</div>
              </div>
            </div>
          )}
          ...
          {step === 3 && done && (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontSize: 15, color: '#6B7280', margin: '0 0 6px' }}>
                Спасибо, <strong>{form.full_name}</strong>!
              </p>
              <p style={{ fontSize: 14, color: '#9CA3AF', margin: 0 }}>
                Мы позвоним вам по номеру <strong>{form.phone}</strong> в течение 30 минут для подтверждения.
              </p>
              <div style={{
          ...
          {step < 3 && (
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              >
                {saving ? 'Отправляем...' : step === 2 ? 'Отправить заявку →' : 'Далее →'}
              </button>
            </div>
          )}
        </div>
        ...
        {step < 3 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginTop: 20, flexWrap: 'wrap' }}>
            {['✅ Без скрытых платежей', '📞 Ответим за 30 мин', '⭐ 5.0 на Google'].map(t => (
