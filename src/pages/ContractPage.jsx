  ...
  return (
    <div style={{minHeight:'100dvh', background:'#F1F5F9', fontFamily:"'Inter',system-ui,sans-serif"}}>
      ...
      <div style={{background:'#0F172A', padding:'14px 16px', display:'flex', alignItems:'center', gap:12, position:'sticky', top:0, zIndex:10}}>
        <button onClick={() => step===0 ? navigate(`/jobs/${id}`) : setStep(s=>s-1)}
      ...
      <div style={{padding:16, maxWidth:540, margin:'0 auto'}}>
        ...
        {step === 0 && <>
          ...
          <SignaturePad
            ...
            onSave={d => setSigs(s=>({...s, estimate_agree:d}))}
          />
        ...
        {step === 1 && <>
          ...
          <SignaturePad
            ...
            onSave={d => setSigs(s=>({...s, start_initials:d}))}
          />

          <button onClick={async () => { await saveToDb(); setStep(2) }}
          ...
          <div style={card}>
            ...
            {packTotal > 0 && (
              <div style={{marginTop:14, padding:'12px 14px', background:'#EFF6FF', borderRadius:12, display:'flex', justifyContent:'space-between', fontWeight:700}}>
                ...
                <span style={{color:'#1D4ED8', fontSize:16}}>${packTotal.toFixed(2)}</span>
              </div>
            )}
          </div>

          <button onClick={() => setStep(3)} style={S('linear-gradient(135deg,#1D4ED8,#6366F1)')}>
            {packTotal > 0 ? `Continue — $${packTotal.toFixed(2)} in materials` : 'Continue — No Packing'} <ChevronRight size={18}/>
          </button>
        </>}
        ...
        {step === 3 && <>
            ...
            {startTime && endTime && (
              <div style={{marginTop:12, padding:'12px 14px', background:'#F8FAFF', borderRadius:12}}>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:13, color:'#64748B', marginBottom:4}}>
                  <span>Raw time worked</span><span>{rawHours.toFixed(2)} hrs</span>
                </div>
                {breakMin > 0 && (
                  <div style={{display:'flex', justifyContent:'space-between', fontSize:13, color:'#64748B', marginBottom:4}}>
                ...
                <div style={{display:'flex', justifyContent:'space-between', fontSize:13, color:'#64748B'}}>
                  ...
                  <span style={{fontWeight:700, color:'#0F172A'}}>{billHours.toFixed(2)} hrs</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={card}>
            ...
            {[
              [`Labor (${moversCount} × ${billHours.toFixed(2)}hrs × $${rate})`, laborTotal],
              ...
              <div key={label} style={{display:'flex', justifyContent:'space-between', fontSize:13, color:'#64748B', marginBottom:6}}>
                <span>{label}</span>
                <span style={{fontWeight:600}}>{amt<0?'-':''}${Math.abs(amt).toFixed(2)}</span>
              </div>
            ))}
            <div style={{borderTop:'2px solid #0F172A', paddingTop:12, marginTop:8, display:'flex', justifyContent:'space-between'}}>
              ...
              <span style={{fontSize:22, fontWeight:900, color:'#1D4ED8'}}>${totalCost.toFixed(2)}</span>
            </div>
            {deposit>0 && (
              <div style={{display:'flex', justifyContent:'space-between', fontSize:13, color:'#059669', marginTop:6, fontWeight:700}}>
                <span>Deposit paid</span><span>−${deposit.toFixed(2)}</span>
              </div>
            )}
            <div style={{marginTop:10, padding:'14px', background:'#FEF2F2', borderRadius:12, display:'flex', justifyContent:'space-between'}}>
              ...
              <span style={{fontSize:22, fontWeight:900, color:'#DC2626'}}>${balanceDue.toFixed(2)}</span>
            </div>
          </div>

          <SignaturePad
            title="Customer Initial — End Time & Total"
            subtitle={`Confirms end time ${endTime} and total $${totalCost.toFixed(2)}`}
            existing={sigs.end_initials}
            onSave={d => setSigs(s=>({...s, end_initials:d}))}
          />
          ...
          <SignaturePad
            ...
            onSave={d => setSigs(s=>({...s, customer_release:d}))}
          />
          ...
          <div style={{textAlign:'center', padding:'40px 0'}}>
            ...
            <div style={{background:'white', borderRadius:16, padding:20, marginBottom:20, textAlign:'left', border:'0.5px solid #E2E8F0'}}>
              ...
              {[
                ...
                ['Hours Billed', `${billHours.toFixed(2)} hrs`],
            ...
            <div style={{display:'flex', flexDirection:'column', gap:10}}>
              <button onClick={() => navigate(`/jobs/${id}/contract-print`)} style={S('#1D4ED8')}>
                🖨 Print Full Contract
              </button>
