...
export default function ContractPage() {
  ...
  const [heavyItems, setHeavyItems] = useState([
    // { name:'Piano', price:'' }, etc — foreman fills in
  ])
  ...
  useEffect(() => {
    getJob(id).then(async j => {
      ...
      // Parse heavy items from notes if any
      if (j.notes) {
        const lines = j.notes.split('\n')
        const heavyLine = lines.find(l => l.startsWith('Heavy items: '))
        if (heavyLine) {
          const items = heavyLine.replace('Heavy items: ', '').split(', ').map(name => ({ name: name.trim(), price: '' }))
          setHeavyItems(items)
        }
      }
  ...
  const saveToDb = async (extra = {}) => {
    // Only saves job fields — signatures are saved immediately via saveSig
    try {
      ...
      <div style={{padding:16, maxWidth:540, margin:'0 auto'}}>
        ...
        {step === 0 && <>
          ...
          <button onClick={async () => {
            ...
            disabled={stepSaving}
            style={S('linear-gradient(135deg,#1D4ED8,#6366F1)')}>
            {stepSaving ? 'Saving...' : <><span>Proceed to Start Time</span><ChevronRight size={18}/></>}
          </button>
        </>}
        ...
        {step === 1 && <>
          ...
          <button onClick={async () => {
              if (stepSaving) return
              setStepSaving(true)
              await saveToDb({ status:'in_progress' })
              setStepSaving(false)
              setStep(2)
            }}
            disabled={!startTime || !sigs.start_initials || stepSaving}
            style={S(!startTime||!sigs.start_initials ? '#CBD5E1' : 'linear-gradient(135deg,#1D4ED8,#6366F1)')}>
            {stepSaving ? 'Saving...' : <><span>Next: Packing Items</span><ChevronRight size={18}/></>}
          </button>
        ...
        {step === 2 && <>
          ...
          {/* Heavy items with price — foreman sets on site */}
          <div style={card}>
            <div style={{fontSize:16, fontWeight:800, color:'#0F172A', marginBottom:4}}>Heavy / Special Items</div>
            <div style={{fontSize:12, color:'#94A3B8', marginBottom:14}}>Set price for each item — leave empty if no charge</div>
            {heavyItems.length === 0 ? (
              <div style={{fontSize:13, color:'#94A3B8', textAlign:'center', padding:'12px 0'}}>No heavy items noted for this job</div>
            ) : heavyItems.map((item, i) => (
              <div key={i} style={{display:'flex', alignItems:'center', gap:10, padding:'10px 0', borderBottom:i<heavyItems.length-1?'0.5px solid #F1F5F9':'none'}}>
                <div style={{flex:1, fontSize:13, fontWeight:600, color:'#0F172A'}}>{item.name}</div>
                <div style={{display:'flex', alignItems:'center', gap:6}}>
                  <span style={{fontSize:14, color:'#64748B'}}>$</span>
                  <input
                    type="number" min="0" placeholder="0"
                    value={item.price}
                    onChange={e => setHeavyItems(items => items.map((it,j) => j===i ? {...it, price:e.target.value} : it))}
                    style={{width:90, border:'1.5px solid #E2E8F0', borderRadius:8, padding:'8px 10px', fontSize:15, fontWeight:700, outline:'none', textAlign:'right', fontFamily:'inherit'}}/>
                </div>
              </div>
            ))}
            <button
              onClick={() => setHeavyItems(items => [...items, {name:'', price:''}])}
              style={{marginTop:12, width:'100%', padding:'10px', borderRadius:10, border:'1.5px dashed #E2E8F0', background:'#F8FAFF', fontSize:13, color:'#64748B', cursor:'pointer', fontFamily:'inherit'}}>
              + Add Item
            </button>
            {heavyTotal > 0 && (
              <div style={{marginTop:12, padding:'12px 14px', background:'#EFF6FF', borderRadius:12, display:'flex', justifyContent:'space-between', fontWeight:700}}>
                <span style={{color:'#1D4ED8'}}>Heavy Items Total</span>
                <span style={{color:'#1D4ED8', fontSize:16}}>${heavyTotal.toFixed(2)}</span>
              </div>
            )}
          </div>

          <button onClick={() => setStep(3)} style={S('linear-gradient(135deg,#1D4ED8,#6366F1)')}>
            <span>{(packTotal+heavyTotal) > 0 ? `Continue — $${(packTotal+heavyTotal).toFixed(2)} extras` : 'Continue'}</span><ChevronRight size={18}/>
          </button>
        </>}
        ...
        {step === 3 && <>
          ...
          <div style={card}>
            ...
            {[
              ...
              ...(heavyTotal>0 ? [['Heavy / Special Items', heavyTotal]] : []),
