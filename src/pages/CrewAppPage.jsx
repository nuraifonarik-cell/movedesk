...
export default function CrewAppPage() {
  ...
  async function loadCrewData(u) {
    ...
    let q = supabase
      ...
      .in('status', ['new', 'quoted', 'scheduled', 'in_progress'])
    ...
    // If no crew member linked OR no assignments yet — show all upcoming jobs
      ...
      <div style={{ padding: '16px 16px 32px' }}>
        ...
        {jobs.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9CA3AF' }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 16, color: '#374151' }}>All clear!</div>
            <div style={{ fontSize: 14, marginTop: 4 }}>No upcoming jobs scheduled</div>
          </div>
        )}
    ...
    <div style={{ background: 'white', borderRadius: 16, marginBottom: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
      ...
      {isExpanded && (
        <div style={{ borderTop: '1px solid #F3F4F6', padding: '16px' }}>
          ...
          <button
            onClick={() => window.location.href = `/jobs/${job.id}/contract`}
