...
// EditPricePreview removed - now inline in modal
      ...
      {editMode && (
        <div style={{position:'fixed', inset:0, zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.5)', padding:16}}>
          <div style={{background:'white', borderRadius:20, padding:24, width:'100%', maxWidth:560, maxHeight:'90dvh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
            ...
            {/* Inline price — reacts to editForm state changes */}
            {(()=>{
              const _apt = editForm.apt_type ?? '1br'
              const _mov = editForm.movers_count ?? 3
              const _dis = editForm.distance_miles ?? 10
              const _hrs = {studio:2,'1br':4,'2br':6,'3br':8,house:10}[_apt] ?? 4
              const _rate = {2:120,3:165,4:210}[_mov] ?? 165
              const _mats = {studio:40,'1br':75,'2br':110,'3br':150,house:200}[_apt] ?? 75
              const _trav = Math.round(_dis * 2.5)
              const _total = _rate*_hrs + _trav + _mats
              return (
                <div style={{background:'#EFF6FF', border:'1.5px solid #BFDBFE', borderRadius:12, padding:'14px 16px', marginBottom:14}}>
                  ...
                  <div style={{display:'flex', flexDirection:'column', gap:6, fontSize:13, color:'#374151'}}>
                    <div style={{display:'flex', justifyContent:'space-between'}}>
                      <span>Labor ({_mov} movers × {_hrs}hrs @ ${_rate}/hr)</span>
                      <span style={{fontWeight:600}}>${(_rate*_hrs).toLocaleString()}</span>
                    </div>
                    <div style={{display:'flex', justifyContent:'space-between'}}>
                      <span>Travel fee</span><span style={{fontWeight:600}}>${_trav}</span>
                    </div>
                    <div style={{display:'flex', justifyContent:'space-between'}}>
                      <span>Materials</span><span style={{fontWeight:600}}>${_mats}</span>
                    </div>
                    <div style={{display:'flex', justifyContent:'space-between', borderTop:'2px solid #BFDBFE', paddingTop:10, marginTop:4}}>
                      ...
                      <span style={{fontWeight:900, fontSize:22, color:'#1D4ED8'}}>${_total.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12}}>
              <div>
                ...
                <select value={editForm.apt_type??'1br'} onChange={e=>setEditForm(f=>({...f,apt_type:e.target.value}))} style={inp}>
              ...
              <div>
                ...
                <select value={editForm.movers_count??3} onChange={e=>setEditForm(f=>({...f,movers_count:+e.target.value}))} style={inp}>
