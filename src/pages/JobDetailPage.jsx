...
import { getJob, updateJob, getCrew, supabase, calcPrice } from '../lib/supabase'
...
// Live price preview — separate component so it re-renders independently
    ...
    <div style={{background:'#EFF6FF', border:'1.5px solid #BFDBFE', borderRadius:12, padding:'14px 16px', marginBottom:14}}>
      ...
      <div style={{display:'flex', flexDirection:'column', gap:6, fontSize:13, color:'#374151'}}>
        ...
        <div style={{display:'flex', justifyContent:'space-between'}}>
          ...
          <span style={{fontWeight:600}}>${p.travelFee}</span>
        </div>
        <div style={{display:'flex', justifyContent:'space-between'}}>
          <span>Materials</span>
          <span style={{fontWeight:600}}>${p.materialsFee}</span>
        </div>
...
export default function JobDetailPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Check which crew members are busy on the same move date
        if (j.move_date) {
  ...
  // Global crew operation lock — prevents any concurrent crew changes
  const [crewLocked, setCrewLocked] = useState(false)

  const toggleCrew = async (memberId) => {
    if (crewLocked) return  // block all clicks while any operation is in progress
    setCrewLocked(true)
    setError('')
    try {
      ...
      if (assigned) {
        ...
        const { error: delErr } = await supabase
          .from('job_assignments').delete().eq('id', assigned.id)
        if (delErr) throw delErr

        const newAssignments = job.assignments.filter(a => a.id !== assigned.id)

        // Check if any foreman remains
        const foremanStillAssigned = newAssignments.some(a => {
          const m = crew.find(c => c.id === a.crew_member_id)
          return (m?.role_type === 'foreman') || (m?.role === 'lead')
        })

        let newStatus = job.status
        if (!foremanStillAssigned && job.status === 'scheduled') {
          await supabase.from('jobs').update({ status:'new' }).eq('id', id)
          newStatus = 'new'
        }

        setJob(j => ({...j, assignments: newAssignments, status: newStatus}))
      ...
      setCrewLocked(false)  // always unlock
    }
  }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      }).eq('id', job.customer_id ?? job.customer?.id)
      ...
      if (!data) throw new Error('Update returned no data — check RLS policies')

      // Update job state immediately with new values (no reload needed)
      setJob(prev => ({
        ...prev,
        ...data,
        customer: {
  ...
  return (
    <div style={{padding:20, fontFamily:"'Inter',system-ui,sans-serif", maxWidth:1000}}>
      ...
      {/* Edit / Delete — not available for completed jobs */}
          ...
          <div style={{background:'white', borderRadius:20, padding:24, width:'100%', maxWidth:560, maxHeight:'90dvh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20}}>
              ...
              <button onClick={() => { setEditMode(false); setError('') }}
                style={{background:'#F1F5F9', border:'none', borderRadius:8, width:30, height:30, cursor:'pointer', fontSize:18, display:'flex', alignItems:'center', justifyContent:'center'}}>
                ✕
              </button>
            </div>
            ...
            {[
              ...
              <div key={key} style={{marginBottom:12}}>
                ...
                <input type={type} value={editForm[key]??''} onChange={e => setEditForm(f=>({...f,[key]:e.target.value}))}
                  placeholder={ph}
                  style={{width:'100%', border:'1px solid #E2E8F0', borderRadius:10, padding:'10px 12px', fontSize:14, outline:'none', boxSizing:'border-box', fontFamily:'inherit'}}/>
              </div>
            ))}
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12}}>
              <div>
                ...
                <select value={editForm.apt_type??'1br'} onChange={e=>setEditForm(f=>({...f,apt_type:e.target.value}))}
                  style={{width:'100%', border:'1px solid #E2E8F0', borderRadius:10, padding:'10px 12px', fontSize:14, outline:'none', background:'white'}}>
              ...
              <div>
                ...
                <select value={editForm.movers_count??3} onChange={e=>setEditForm(f=>({...f,movers_count:+e.target.value}))}
                  style={{width:'100%', border:'1px solid #E2E8F0', borderRadius:10, padding:'10px 12px', fontSize:14, outline:'none', background:'white'}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <textarea value={editForm.notes??''} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))}
                ...
                style={{width:'100%', border:'1px solid #E2E8F0', borderRadius:10, padding:'10px 12px', fontSize:14, outline:'none', resize:'none', boxSizing:'border-box', fontFamily:'inherit'}}/>
            </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 280px', gap:16}} className="job-detail-grid">
        ...
        <div style={{display:'flex', flexDirection:'column', gap:14}}>
          ...
          <div style={card}>
            <h2 style={sectionTitle}><Users size={14} color="#94A3B8"/>
              {['in_progress','completed','cancelled'].includes(job.status) ? 'Assigned Crew' : 'Assign Crew'}
            </h2>
            {['in_progress','completed','cancelled'].includes(job.status) ? (
              <div style={{padding:'10px 14px', background:'#F8FAFF', borderRadius:10, fontSize:12, color:'#64748B', marginBottom:8}}>
                {job.status === 'in_progress' && '🔒 Job is in progress — crew cannot be changed'}
                ...
                {[
                  ...
                  <div key={group.label} style={{display:'flex', flexDirection:'column', gap:6}}>
                    ...
                    {group.members.length > 0 && (
                      <>
                      {/* Busy warnings */}
                      {group.members.filter(m => busyMap[m.id] && !isAssigned(m.id)).map(m => (
                        <div key={m.id} style={{fontSize:11, color:'#D97706', background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:8, padding:'5px 10px', marginBottom:4, display:'flex', alignItems:'center', gap:5}}>
                          ⚠️ <strong>{m.full_name}</strong> is assigned to {busyMap[m.id]?.customer?.full_name} on this date
                        </div>
                      ))}
                      <select
                        multiple={!group.single}
                        size={group.single ? 1 : Math.min(group.members.length, 4)}
                        disabled={['in_progress','completed','cancelled'].includes(job.status)}
                      onChange={e => {
        ...
        <div style={{display:'flex', flexDirection:'column', gap:14}}>
          ...
          {/* Pricing — always shows latest saved values */}
          <div style={card}>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:8, fontSize:13}}>
              ...
              <div style={{borderTop:'2px solid #0F172A', paddingTop:10, marginTop:4, display:'flex', justifyContent:'space-between', fontWeight:800, fontSize:18, color:'#0F172A'}}>
                <span>Est. Total</span>
