...
export default function JobDetailPage() {
  ...
  const assignCrew = async (removeId, addId, role) => {
    ...
    if (!removeId && !addId) return
    // Hard lock — processingRef prevents re-entry even during React re-renders
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        ...
                        <select disabled={isLocked||crewLocked} value={curId}
                          onChange={e => { const v = e.target.value; if (v !== curId) setTimeout(() => assignCrew(curId||null, v||null, 'foreman'), 0) }}
                          style={{width:'100%',border:`1.5px solid ${curId?'#1D4ED8':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:curId?'#EFF6FF':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',color:'#0F172A'}}>
                          <option value="">— Select Foreman —</option>
                          {members.map(m=>(
                            <option key={m.id} value={m.id}>{m.full_name}{m.phone?` · ${m.phone}`:''}{busyMap[m.id]?' ⚠️':''}</option>
                          ))}
                        </select>
                        {curId && busyMap[curId] && <div style={{fontSize:11,color:'#D97706',marginTop:4}}>⚠️ This foreman has another job on this day</div>}
                      </div>
                    )
                  })()}

                  {/* HELPERS — dropdown multi-select */}
                  {(() => {
                    ...
                    return (
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
                          ...
                          {assignedHelpers.length > 0 && <span style={{fontSize:10,color:'#059669',background:'#D1FAE5',padding:'1px 8px',borderRadius:20,fontWeight:700}}>{assignedHelpers.length} selected</span>}
                          {crewLocked && <span style={{fontSize:10,color:'#94A3B8'}}>saving...</span>}
                        </div>
                        <div style={{border:'1px solid #E2E8F0',borderRadius:10,overflow:'hidden'}}>
                          {members.map((m, i) => {
                            ...
                            return (
                              <div key={m.id}
                                onClick={() => !isLocked && !crewLocked && toggleHelper(m.id)}
                                style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:i<members.length-1?'0.5px solid #F1F5F9':'none',background:checked?'#F0FDF4':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',transition:'background 0.1s'}}>
                                <div style={{width:20,height:20,borderRadius:6,border:`2px solid ${checked?'#059669':'#CBD5E1'}`,background:checked?'#059669':'white',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all 0.15s'}}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        ...
                        <select disabled={isLocked||crewLocked} value={curId}
                          onChange={e => { const v = e.target.value; if (v !== curId) setTimeout(() => assignCrew(curId||null, v||null, 'driver'), 0) }}
                          style={{width:'100%',border:`1.5px solid ${curId?'#D97706':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:curId?'#FFFBEB':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',color:'#0F172A'}}>
