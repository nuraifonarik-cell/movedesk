...
export default function JobDetailPage() {
  ...
  const toggleCrew = async (memberId) => {
    ...
    try {
      ...
      if (assigned) {
        ...
        const foremanLeft = newA.some(a => { const m = crew.find(c => c.id === a.crew_member_id); return m?.role_type==='foreman'||m?.role==='lead' })
        let ns = job.status
        if (!foremanLeft && job.status==='scheduled') { await supabase.from('jobs').update({status:'new'}).eq('id',id); ns='new' }
        setJob(j => ({...j, assignments:newA, status:ns}))
      } else {
        const member = crew.find(c => c.id === memberId)
        const isForeman = member?.role_type==='foreman'||member?.role==='lead'
        const { data, error: e } = await supabase.from('job_assignments').insert({job_id:id,crew_member_id:memberId}).select('*, crew_member:crew_members(*)').single()
        if (e) throw e
        const newA = [...(job.assignments??[]), data]
        let ns = job.status
        if (isForeman && job.status==='new') { await supabase.from('jobs').update({status:'scheduled'}).eq('id',id); ns='scheduled' }
        setJob(j => ({...j, assignments:newA, status:ns}))
      }
    } catch (e) { setError('Crew update failed: '+(e?.message??'')) }
    finally { setCrewLocked(false) }
  }
  ...
  const saveEdit = async () => {
    ...
    try {
      const p = calcPrice({aptType:editForm.apt_type, moversCount:editForm.movers_count, distanceMiles:editForm.distance_miles})
      await supabase.from('customers').update({full_name:editForm.full_name,phone:editForm.phone,email:editForm.email||null}).eq('id',job.customer?.id)
      const { data, error: e } = await supabase.from('jobs').update({
        from_address:editForm.from_address, to_address:editForm.to_address, move_date:editForm.move_date,
        apt_type:editForm.apt_type, distance_miles:editForm.distance_miles, movers_count:editForm.movers_count,
        notes:editForm.notes||null, estimated_hours:p.hours, base_rate:p.rate,
        travel_fee:p.travelFee, materials_fee:p.materialsFee, total_price:p.total,
      }).eq('id',id).select().single()
      if (e) throw e
      setJob(prev => ({...prev,...data,customer:{...prev.customer,full_name:editForm.full_name,phone:editForm.phone,email:editForm.email||prev.customer?.email}}))
      setEditMode(false)
  ...
  // Live price — computed directly from editForm state at render time
  const eApt  = editForm.apt_type??'1br'
  const eMov  = Number(editForm.movers_count??3)
  const eDis  = Number(editForm.distance_miles??10)
  const eHrs  = {studio:2,'1br':4,'2br':6,'3br':8,house:10}[eApt]??4
  const eRate = {2:120,3:165,4:210}[eMov]??165
  const eMats = {studio:40,'1br':75,'2br':110,'3br':150,house:200}[eApt]??75
  const eTrav = Math.round(eDis*2.5)
  const eTotal = eRate*eHrs + eTrav + eMats
    ...
    <div style={{padding:20,fontFamily:"'Inter',system-ui,sans-serif",maxWidth:1000}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 280px',gap:16}} className="job-detail-grid">
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={card}>
            <h2 style={sectionTitle}><Users size={14} color="#94A3B8"/>{isLocked?'Assigned Crew':'Assign Crew'}{crewLocked&&<span style={{fontSize:11,color:'#94A3B8',marginLeft:6}}>Saving...</span>}</h2>
            {isLocked&&<div style={{padding:'10px 14px',background:'#F8FAFF',borderRadius:10,fontSize:12,color:'#64748B',marginBottom:12}}>🔒 {job.status==='in_progress'?'Job in progress — crew is locked':job.status==='completed'?'Job completed — crew is locked':'Job cancelled'}</div>}
            {crew.length===0?<p style={{fontSize:13,color:'#94A3B8'}}>No active crew. <Link to="/crew" style={{color:'#1D4ED8'}}>Add crew →</Link></p>
            :crewGroups.length===0?<p style={{fontSize:13,color:'#94A3B8'}}>No crew members found.</p>
            :(
              <div style={{display:'flex',flexDirection:'column',gap:16}}>
                {crewGroups.map(group=>(
                  <div key={group.label}>
                    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
                      <div style={{width:22,height:22,borderRadius:6,background:group.bg,display:'flex',alignItems:'center',justifyContent:'center'}}><group.icon size={12} color={group.color}/></div>
                      <span style={{fontSize:12,fontWeight:700,color:group.color}}>{group.label}</span>
                    </div>
                    <div style={{display:'flex',flexDirection:'column',gap:6}}>
                      {group.members.map((m,mi)=><CrewButton key={m.id} member={m} assigned={isAssigned(m.id)} busy={!!busyMap[m.id]&&!isAssigned(m.id)} locked={isLocked||crewLocked} color={group.color} bg={group.bg} onToggle={toggleCrew} index={mi}/>)}
                    </div>
                  </div>
                ))}
              </div>
      ...
      {editMode && (
        <div style={{position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.5)',padding:16}}>
          <div style={{background:'white',borderRadius:20,padding:24,width:'100%',maxWidth:520,maxHeight:'90dvh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
            ...
            <div style={{marginBottom:14}}>
              <label style={{display:'block',fontSize:12,fontWeight:600,color:'#64748B',marginBottom:5}}>Distance (miles)</label>
              <input type="number" min="1" value={editForm.distance_miles??10} onChange={e=>setEdit('distance_miles',+e.target.value)} style={inp}/>
            </div>

            {/* PRICE — right below the controls that affect it */}
            <div style={{background:'#EFF6FF',border:'1.5px solid #BFDBFE',borderRadius:12,padding:'14px 16px',marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:'#1D4ED8',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:10}}>Price Estimate</div>
              <div style={{display:'flex',flexDirection:'column',gap:6,fontSize:13,color:'#374151'}}>
                <div style={{display:'flex',justifyContent:'space-between'}}><span>Labor ({eMov} movers × {eHrs}hrs @ ${eRate}/hr)</span><span style={{fontWeight:700}}>${(eRate*eHrs).toLocaleString()}</span></div>
                <div style={{display:'flex',justifyContent:'space-between'}}><span>Travel fee</span><span style={{fontWeight:700}}>${eTrav}</span></div>
                <div style={{display:'flex',justifyContent:'space-between'}}><span>Materials</span><span style={{fontWeight:700}}>${eMats}</span></div>
                <div style={{display:'flex',justifyContent:'space-between',borderTop:'2px solid #BFDBFE',paddingTop:10,marginTop:4}}>
                  ...
                  <span style={{fontWeight:900,fontSize:22,color:'#1D4ED8'}}>${eTotal.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <textarea value={editForm.notes??''} onChange={e=>setEdit('notes',e.target.value)} rows={3} placeholder="Elevator? Piano? Fragile items?" style={{...inp,resize:'none'}}/>
            </div>
