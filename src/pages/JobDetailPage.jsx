            ...
            {(() => {
              ...
              return (
                <div style={{background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:12, padding:'12px 16px', marginBottom:12}}>
                  <div style={{fontSize:11, fontWeight:700, color:'#1D4ED8', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Updated Price Estimate</div>
                  <div style={{display:'flex', gap:16, flexWrap:'wrap', fontSize:13, color:'#374151', marginBottom:8}}>
                    <span>${p.rate}/hr × {p.hours}hrs = <strong>${p.rate*p.hours}</strong></span>
                    <span>Travel: <strong>${p.travelFee}</strong></span>
                    <span>Materials: <strong>${p.materialsFee}</strong></span>
                  </div>
                  <div style={{display:'flex', justifyContent:'space-between', borderTop:'1px solid #BFDBFE', paddingTop:8}}>
                    <span style={{fontWeight:700, color:'#0F172A', fontSize:15}}>New Total</span>
                    <span style={{fontWeight:900, color:'#1D4ED8', fontSize:18}}>${p.total.toLocaleString()}</span>
                  </div>
                </div>
              )
            })()}
