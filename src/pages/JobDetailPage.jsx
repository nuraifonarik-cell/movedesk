...
export default function JobDetailPage() {
  ...
  if (!job)    return <div className="p-8 text-sm text-gray-400">Job not found. <Link to="/jobs" className="text-brand-600">← Back</Link></div>
    ...
    <div className="p-4 md:p-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Job created {format(new Date(job.created_at), 'd MMM yyyy')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="md:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Truck size={15} className="text-gray-400" /> Move Details
            </h2>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <div className="text-xs text-gray-500 mb-1 flex items-center gap-1"><Calendar size={11} /> Date</div>
                <div className="font-medium text-gray-900">{format(new Date(job.move_date), 'd MMMM yyyy')}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Home Type</div>
                <div className="font-medium text-gray-900">{APT_LABELS[job.apt_type] ?? job.apt_type}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Distance</div>
                <div className="font-medium text-gray-900">{job.distance_miles} miles</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Crew</div>
                <div className="font-medium text-gray-900">{job.movers_count} movers × {job.estimated_hours} ч</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <ClipboardList size={15} className="text-gray-400" /> Job Status
            </h2>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Users size={15} className="text-gray-400" /> Assign Crew
            </h2>
            {crew.length === 0 ? (
              <p className="text-sm text-gray-400">No active crew members. <Link to="/crew" className="text-brand-600">Add →</Link></p>
            ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <DollarSign size={15} className="text-gray-400" /> Pricing
            </h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-gray-600">
                <span>Labor ({job.movers_count} movers × {job.estimated_hours}ч)</span>
