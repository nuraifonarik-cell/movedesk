    ...
    <div style={{padding:20, fontFamily:"'Inter',system-ui,sans-serif", maxWidth:1000}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 280px', gap:16}} className="job-detail-grid">
        ...
        <div style={{display:'flex', flexDirection:'column', gap:14}}>
          ...
          {/* Actions — shown based on job status */}
          <div style={card}>
            <h2 style={sectionTitle}>Actions</h2>
            <div style={{display:'flex', flexDirection:'column', gap:8}}>

              {/* NEW: just call customer, no contract yet */}
              {job.status === 'new' && (
                <div style={{padding:'10px 14px', background:'#FEF3C7', borderRadius:10, fontSize:12, color:'#92400E', fontWeight:600}}>
                  ⚠️ Assign crew and schedule before opening contract
                </div>
              )}

              {/* SCHEDULED or later: contract available */}
              {['scheduled','in_progress','completed'].includes(job.status) && (
                <button onClick={() => navigate(`/jobs/${id}/contract`)}
                  ...
                  <FileText size={14}/> {job.status === 'in_progress' ? 'Continue Contract' : job.status === 'completed' ? 'View Contract' : 'Open Contract'}
                </button>
              )}

              {/* Print contract — only after job started */}
