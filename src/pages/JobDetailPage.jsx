                        ...
                        <div style={{display:'flex',gap:8}}>
                          <select disabled={isLocked||crewLocked} value={curId}
                            onChange={e => {
                              ...
                              if (!v || v === curId) return  // ignore empty or same
                              setTimeout(() => assignCrew(curId||null, v, 'foreman'), 0)
                            }}
                            style={{flex:1,border:`1.5px solid ${curId?'#1D4ED8':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:curId?'#EFF6FF':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',color:'#0F172A'}}>
                          ...
                          {curId && !isLocked && (
                            <button
                              disabled={crewLocked}
                              onClick={() => !crewLocked && setTimeout(() => assignCrew(curId, null, 'foreman'), 0)}
                              style={{padding:'0 12px',borderRadius:10,border:'1px solid #FECACA',background:'#FEF2F2',color:'#DC2626',fontSize:12,cursor:crewLocked?'not-allowed':'pointer',flexShrink:0}}
                        ...
                        <div style={{display:'flex',gap:8}}>
                          <select disabled={isLocked||crewLocked} value={curId}
                            onChange={e => {
                              const v = e.target.value
                              if (!v || v === curId) return
                              setTimeout(() => assignCrew(curId||null, v, 'driver'), 0)
                            }}
                            style={{flex:1,border:`1.5px solid ${curId?'#D97706':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:curId?'#FFFBEB':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',color:'#0F172A'}}>
                          ...
                          {curId && !isLocked && (
                            <button
                              disabled={crewLocked}
                              onClick={() => !crewLocked && setTimeout(() => assignCrew(curId, null, 'driver'), 0)}
                              style={{padding:'0 12px',borderRadius:10,border:'1px solid #FECACA',background:'#FEF2F2',color:'#DC2626',fontSize:12,cursor:crewLocked?'not-allowed':'pointer',flexShrink:0}}
                              title="Remove driver">
                              ✕
                            </button>
