        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={card}>
            ...
            {crew.length === 0
              ...
              : (
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {/* FOREMAN — single select */}
                  {(() => {
                    const foremans = crew.filter(m => getRoleKey(m) === 'foreman')
                    const assigned = job.assignments?.find(a => foremans.some(f => f.id === a.crew_member_id))
                    const assignedId = assigned?.crew_member_id ?? ''
                    if (foremans.length === 0) return null
                    return (
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
                          <UserCheck size={13} color="#1D4ED8"/>
                          <span style={{fontSize:12,fontWeight:700,color:'#1D4ED8'}}>Foreman</span>
                          {busyMap[assignedId] && <span style={{fontSize:10,color:'#D97706',background:'#FFFBEB',padding:'1px 7px',borderRadius:20}}>⚠️ Busy this day</span>}
                        </div>
                        <select
                          ...
                          onChange={async e => {
                            ...
                            // Remove current foreman first
                            if (assigned) await toggleCrew(assigned.crew_member_id)
                            // Add new foreman if selected
                            if (newId) await toggleCrew(newId)
                          }}
                          style={{width:'100%',border:`1.5px solid ${assignedId?'#1D4ED8':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:assignedId?'#EFF6FF':'white',cursor:isLocked?'not-allowed':'pointer',color:'#0F172A'}}>
                          <option value="">— Select Foreman —</option>
                          {foremans.map(m => (
                            <option key={m.id} value={m.id}>
                              {m.full_name}{m.phone ? ` · ${m.phone}` : ''}{busyMap[m.id] ? ' ⚠️' : ''}
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    return (
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
                          ...
                          <span style={{fontSize:10,color:'#94A3B8'}}>(select multiple)</span>
                        </div>
                        <div style={{display:'flex',flexDirection:'column',gap:6}}>
                          {helpers.map(m => {
                            ...
                            return (
                              <label key={m.id} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:10,border:`1.5px solid ${checked?'#059669':'#E2E8F0'}`,background:checked?'#F0FDF4':'white',cursor:isLocked||crewLocked?'not-allowed':'pointer',opacity:isLocked?0.6:1}}>
                                <input type="checkbox" checked={checked} disabled={isLocked||crewLocked}
                                  onChange={() => !crewLocked && toggleCrew(m.id)}
                                  style={{width:16,height:16,accentColor:'#059669',cursor:'pointer',flexShrink:0}}/>
                                <div style={{flex:1}}>
                                  <span style={{fontSize:13,fontWeight:600,color:'#0F172A'}}>{m.full_name}</span>
                                  <span style={{fontSize:11,color:'#94A3B8',marginLeft:8}}>{m.phone}</span>
                                </div>
                                {busy && <span style={{fontSize:10,color:'#D97706'}}>⚠️ busy</span>}
                              </label>
                            )
                          })}
                        </div>
                  ...
                  {(() => {
                    ...
                    const assigned = job.assignments?.find(a => drivers.some(d => d.id === a.crew_member_id))
                    const assignedId = assigned?.crew_member_id ?? ''
                    ...
                    return (
                      <div>
                        ...
                        <select
                          ...
                          style={{width:'100%',border:`1.5px solid ${assignedId?'#D97706':'#E2E8F0'}`,borderRadius:10,padding:'10px 12px',fontSize:13,fontFamily:'inherit',outline:'none',background:assignedId?'#FFFBEB':'white',cursor:isLocked?'not-allowed':'pointer',color:'#0F172A'}}>
