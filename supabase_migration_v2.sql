...
begin
  if not exists (
    select 1 from pg_policies where policyname = 'Public can create customers' and tablename = 'customers'
  ) then
    execute 'create policy "Public can create customers" on public.customers for insert with check (true)';
  end if;
  if not exists (
    select 1 from pg_policies where policyname = 'Public can create jobs' and tablename = 'jobs'
  ) then
    execute 'create policy "Public can create jobs" on public.jobs for insert with check (true)';
  end if;
end
$$;
